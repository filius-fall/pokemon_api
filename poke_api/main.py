...
class Pokemon:
    ...
    def __str__(self):
        stats = [[i,self.get_pokemo_details(self.name)['stats'][i]] for i in self.get_pokemo_details(self.name)['stats']]
    ...
    def damage_by_each_move(self,move_name):
        ...
        poke_details = self.get_pokemo_details(self.name)
        damage = move_damages(move_name)['damage']

        return int(damage)

    
    def get_pokemo_details(self,poke_name):
